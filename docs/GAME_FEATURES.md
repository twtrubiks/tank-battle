...
經典 FC Battle City (1985) 復刻版坦克大戰，使用 Phaser.js 4 開發。
...
- **完整測試覆蓋**: 298 個單元測試全部通過
- **性能優化**: 物件池、碰撞分組，穩定 60 FPS
- **設計模式**: 狀態機、工廠、觀察者、單例、物件池
- **智能 AI**: 狀態機驅動行為 + A* 尋路 + 統一移動意圖出口（視線檢測、預測射擊、攻擊保持距離）
- **UI 設計系統**: Style A 金橘像素復古風（Press Start 2P + Cubic 11 中英雙像素字型 + 集中式 `UITheme.js`），HUD 上下分層 44/36px，可玩區域 832×832 完整不被遮擋
...
- **構建**: Webpack 5 + Babel
- **測試**: Jest
- **代碼行數**: 約 8,500 行（含 2,600 行測試，使用 cloc 計算）
